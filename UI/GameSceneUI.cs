...
using DungeonSlime.Utilities;
using DungeonSlime.Utilities.Graphics;
...
public class GameSceneUI : ContainerRuntime
{
    ...
    private const string SCORE_FORMAT = "Score: {0,6} (Highest: {1,6})";
    ...
    private readonly TextRuntime _scoreText;

    // The text runtime used to display instructions to the player.
    private readonly TextRuntime _instructionText;
    ...
    public GameSceneUI()
    {
        ...
        // Create the text that will display the players score and add it as a child to
        // the corresponding container. The instruction text is also created and added
    ...
    private static TextRuntime CreateScoreText()
    {
        ...
        text.Text = string.Format(SCORE_FORMAT, 0, 0);

        return text;
    }

    private static TextRuntime CreateInstructionText()
    {
        TextRuntime text = new();
        ...
        text.CustomFontFile = @"fonts/04b_30.fnt";
        text.FontScale = 0.25f;
        text.Text = "Arrow keys or WASD = Move, Esc = Pause";
    ...
    /// <param name="highScore">The high score to display.</param>
    public void UpdateScoreText(int score, int highScore)
    {
        _scoreText.Text = string.Format(SCORE_FORMAT, score, highScore);
    }
